...
int main(int argc, char **argv) {
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    double loc[2] = wb_gps_get_values(gps)[0,2];
    double dist1 = sqrt((point1[0]-loc[0])*(point1[0]-loc[0])-(point1[1]-loc[1])*(point1[1]-loc[1]));
    double dist2 = sqrt((point2[0]-loc[0])*(point2[0]-loc[0])-(point2[1]-loc[1])*(point2[1]-loc[1]));
    if (dist1 <= dist2) {
      switch (loc[0]){
        case (loc[0]<point1[0]):
