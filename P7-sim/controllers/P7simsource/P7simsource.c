...
int main(int argc, char **argv) {
  ...
  // Variables.
  double target_altitude = 5.0;  // The target altitude. Can be changed by the user.
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double point1[2] = {-7.36, -1.44};
    const double point2[2] = {-4.78, -3.6};
    
    double loc[2] = {wb_gps_get_values(gps)[0],wb_gps_get_values(gps)[2]};
    double start[2] = {0.0, 0.0};
    double dist1 = sqrt((point1[0]-start[0])*(point1[0]-start[0])-(point1[1]-start[1])*(point1[1]-start[1]));
    double dist2 = sqrt((point2[0]-start[0])*(point2[0]-start[0])-(point2[1]-start[1])*(point2[1]-start[1]));
    if (dist1 <= dist2) {
      printf("dist1 <= dist2");
        while (loc[0]<point1[0]){
        ...
        while (loc[0]>point1[0]){
          pitch_disturbance = -2.0;
          loc[0] = wb_gps_get_values(gps)[0];
          if ((loc[0]==point1[0])){
            pitch_disturbance = 0.0;
            }
          }
        while (loc[1]<point1[1]){
